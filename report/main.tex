...
We compare the MAF estimates of stellar density in various density regimes to Dark Energy Camera (DECam)  data, taken with the 4-m Cerro Tololo Inter-American Observatory telescope (CTIO)\footnote{see \url{http://www.ctio.noao.edu/noao/node/1033}} as part of the DECam Plane Survey~\cite{schlafly2017}.   Fig.~\ref{fig:decaps_fields} shows locations of all DECAPS fields in the single-epoch image database. The database contains information from all DECAPS single-epoch image headers, and was used to select DECAPS fields with single-epoch depth similar to the single-visit depth of 30 sec LSST exposure. Thus we required : 
\begin{itemize}
...
We cross-matched the DECAPS image database with the  stellar density information contained in MAF healpixels. Each DECAPS image plane is tiled by a mosaic of 62 CCDs (see Fig.~\ref{fig:decam_mosaic_ccd}).  A rectangle  composed of four extreme borders of each image plane has a diagonal of approximately 1.5 \degree. Given the coordinates of the center of each DECAPS field we found a corresponding nearest healpixel within 0.5 \degree. 
