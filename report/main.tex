...
\setDocStatus{draft}
\setDocAbstract{%
A report on the performance of current LSST Stack pipelines in crowded stellar fields. We use the DECAPS data to define the photometry and astrometry quality assurance metrics. The mean completeness of LSST detections compared to DECAPS between 18-20 mag is at the level of 82\% for regions with 200 000 sources per sq. deg. (top 10 \% density). For the same density, the completeness drops to 50\% at 21.2 mag.  
...
\begin{table}
\centering
\caption{Pairs of visits at two different epochs. RA and DEC are in degrees. The separation in arcseconds is between the center of each exposure frame. The last column shows the time difference between the two visits in  days.}
