...
\begin{table}
\centering
\caption{Dependence of the stellar count in Galfast simulation  on the limiting magnitude. All counts are in billions:  $10^{9}$. $N_{all}$ is the count in all healpixels, $N_{top 1\% (10\%)}$ in the pixels in top 1\% (10\%) density, $N_{main}$ in the main survey area $\delta>-65\degree$, $\delta < 5 \degree$. $N_{main-conf.}$ is the count in the main survey area excluding the confusion zone. }
\label{tab:galfast_counts}
\begin{tabular}{ ccccccc} 
\hline
mag & $N_{all}$ &  $N_{top 1\%}$ & $N_{top 10\%}$ & $N_{main}$ & $N_{main-conf.}$ \\ 
\hline
...
\begin{table}
\centering
\caption{The area and number of stars in pixels with density < $\rho$ [sources per sq.deg] in the main survey area, excluding the confusion zone. Note that at r<24.5, all healpixels at densities at or above $\approx$1 mln stars per sq.deg. are in  the confusion zone, so increasing the density cutoff does not include more healpixels. However, for r<27.5 there are regions outside the confusion zone even at $\rho$ > 1 mln sources per sq.deg. }
\label{tab:galfast_densities}
\begin{tabular}{ccccc}
$\rho$ & A(r<24.5) & N(r<24.5) & A(r<27.5) & N(r<27.5) \\
35 k & 13002.34 & 161485401 & 10686.73 & 179898977 \\
100 k & 16226.9 & 354829923 & 14492.08 & 405990021 \\
200 k & 17631.88 & 553306692 & 16300.76 & 663514129 \\
300 k & 18235.33 & 701100602 & 17112.36 & 862753997 \\
400 k & 18581.96 & 821170347 & 17616.77 & 1036741614 \\
600 k & 18920.2 & 985810603 & 18195.05 & 1321063932 \\
700 k & 19003.29 & 1039541387 & 18378.01 & 1439524570 \\
800 k & 19053.64 & 1077311263 & 18519.85 & 1545442354 \\
1 mln & 19084.7 & 1103911114 & 18742.27 & 1743324923 \\
2 mln & 19084.7 & 1103911114 & 19084.7 & 2192737211 \\
\end{tabular}
\end{table}
...
The median error reported by either pipeline for either epoch is a measure of Poisson noise - the expected uncertainty in a repeated measurement.  If $e_{1L}$ and $e_{2L}$ are LSST-reported error measurements for a given source for the two epochs,  the quadrature-mean is  
...
The scatter between the two pipelines calculated either for epoch1  $\sigma_{G}(D1,L1)$, or epoch2 $\sigma_{G}(D2,L2)$. We find the quadrature-mean:
...
The scatter between the two epochs within the same pipeline:   $\sigma_{G}(D1,D2)$, or $\sigma_{G}(L1,L2)$, consists of  noise $\sigma_{E} $ and the systematic offset $\sigma_{S}$ : 

\begin{equation}
\sigma_{LL}^{2} = \sigma_{S}^{2} + \sigma_{E}^{2}
\end{equation}

Thus we calculate the systematic offset for LSST as:
...
We consider two properties of successful astrometry: repeatability and accuracy. 

First, the internal consistency, measuring the repeatability of astrometric measurement epoch to epoch. For both LSST and DECAPS, we find the spread of differences in measured position among cross-matched sources: $\sigma_{G}(\Delta \alpha)$, $\sigma_{G}(\Delta \delta)$. Ideally, the median difference should be centered on 0, with the smallest possible spread. The astrometric repeatability for LSST is illustrated on Fig.~\ref{fig:ra_dec_mag}. We restrict the comparison to bright stars only ( m<19 mag) on the following Fig.~\ref{fig:ra_dec_lsst_lsst}. For DECAPS the same visits are shown on Fig.~\ref{fig:ra_dec_decaps_decaps}. There is not much difference that can be seen by eye between LSST vs DECAPS repeatability for this pair of visits, but calculating the  spread of $\Delta \alpha$, $\Delta \delta$ for all visits shows that overall LSST  is more accurate, with $\sigma_{G}(\Delta \alpha, \delta)$ on the order of 10-30 marcsec (Fig.~\ref{fig:ra_dec_summary})
  

Second, pipeline-to-pipeline differences : see Fig.~\ref{fig:ra_dec_lsst_decaps}. 
...
%\vskip -0.15in
\caption{The difference of LSST processing for  RA,DEC for visits 644074,644070 : a top 1\% region according to Galfast, with DECAPS measured 590 704 sources per sq.deg. We select sources brighter than 19 magnitude.  For all other pairs the  offsets are also centered on zero with similar spread (Fig.~\ref{fig:ra_dec_summary})}
...
%\vskip -0.15in
\caption{Summary of LSST and DECAPS astrometric repeatability, with magnitude cutoff at 19 mag.}
...
We find that the LSST image processing pipelines perform well compared to DECAPS pipelines that were specifically designed for crowded field photometry.  The mean 18-20 mag completeness of LSST to DECAPS detections even at 5\% density level, that approximately traces the contours of the Galactic Confusion Zone, is 85\%.  
